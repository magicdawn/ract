...
const Parser = module.exports = class Parser {
  constructor(str, filename, included) {
  ...
  parse() {
    ...
    while (this.tokens.length) {
      ...
      // block
        ...
        if (exists) {
          throw this.lexer.error('duplicate block definition', n.pos)
        }
  ...
  parseFile() {
    ...
    if (this.extending) {
      switch (this.cur().type) {
        case 'extend':
          throw this.lexer.error('u can only extend once', this.cur().pos)
        case 'block':
        ...
        default:
          throw this.lexer.error('unexpected token on line ' + this.cur().lineno, this.cur().pos)
      }
    }
    ...
    if (this.included) {
      switch (this.cur().type) {
        ...
        case 'block':
        case 'blockDefine':
          throw this.lexer.error(`token ${ this.cur().type } are not allowed in included files`, this.cur().pos)
        case 'comment':
        case 'htmlComment':
        case 'text':
        case 'interpolation':
