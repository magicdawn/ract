...
class BaseParser {
  ...
  parse() {
    if (!this.lexer) {
      throw new Error('this.lexer not found, U should init your Lexer in constructor')
    }
...
function error(message: string, pos: number) {
  return new SyntaxError(message, {
    input: this.input,
    filename: this.filename,
    pos,
...
export { BaseLexer, BaseParser, SyntaxError, error }
